...
public class SimpleRelocator implements Relocator {
     ...
     * </ul>
     */
    private static final Pattern RX_ENDS_WITH_DOT_OR_SPACE_WITHOUT_OPERATOR = Pattern.compile("(\\.|[^<>?:+\\-*/^|&] )$");
    ...
    private String shadeSourceWithExcludes(
        ...
        for (int i = 0, snippetsLength = snippets.length; i < snippetsLength; i++) {
            ...
            if (isFirstSnippet) {
                ...
                boolean afterDotSlashSpace = RX_ENDS_WITH_DOT_OR_SPACE_WITHOUT_OPERATOR
